    ...
    <div className="min-h-screen bg-[#0D1117] text-white p-8">
      <div className="max-w-4xl mx-auto">
        <header className="mb-8">
          ...
          <p className="text-gray-400">Generate DCF valuations and fundamental analysis</p>
        </header>

        <div className="bg-[#161B22] rounded-lg p-6 mb-8 border border-gray-800">
          <label className="block text-sm font-medium mb-2">Enter Ticker Symbol</label>
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 px-4 py-3 bg-[#0D1117] border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 text-white"
            />
            <button
              onClick={handleAnalysis}
              disabled={loading}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 rounded-lg font-semibold flex items-center gap-2 transition"
            >
        ...
        {results && (
          <div className="space-y-6">
            <div className="bg-[#161B22] rounded-lg p-6 border border-gray-800">
              <h2 className="text-2xl font-bold mb-4">{results.company_name}</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  <p className="text-gray-400 text-sm">Upside</p>
            ...
            <div className="bg-[#161B22] rounded-lg p-6 border border-gray-800">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-[#0D1117] p-4 rounded-lg">
                  <p className="text-gray-400 text-sm mb-1">Enterprise Value</p>
                  <p className="font-semibold text-xl">
                    ${(results.dcf_results.enterprise_value / 1e9).toFixed(2)}B
                  </p>
                </div>
                <div className="bg-[#0D1117] p-4 rounded-lg">
                  <p className="text-gray-400 text-sm mb-1">Equity Value</p>
                  <p className="font-semibold text-xl">
                    ${(results.dcf_results.equity_value / 1e9).toFixed(2)}B
                  </p>
                </div>
                <div className="bg-[#0D1117] p-4 rounded-lg">
                  <p className="text-gray-400 text-sm mb-1">PV of FCF</p>
                  <p className="font-semibold text-xl">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {results.dcf_results.projections.slice(0, 5).map((proj: any) => (
                      <tr key={proj.Year} className="border-b border-gray-800">
                        <td className="py-2">{proj.Year}</td>
                        <td className="text-right">${(proj.Revenue / 1e9).toFixed(2)}B</td>
                        <td className="text-right">${(proj.EBITDA / 1e9).toFixed(2)}B</td>
                        <td className="text-right">${(proj.FCF / 1e9).toFixed(2)}B</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
